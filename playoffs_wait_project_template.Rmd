...
predict(linearModel_west, data.frame(net_rtg = -2))

linearModel_east = lm(missed_seasons~net_rtg, data=East)
summary(linearModel_east)
predict(linearModel_east, data.frame(net_rtg = -2))
...
Wes <- merged_stint[merged_stint$conference == 'West',]
mean(Wes$stint_length)
Eas <- merged_stint[merged_stint$conference == 'East',]
mean(Eas$stint_length)
...
**ANSWER 13 (b):**  The proportion of the permutation plot has 0.1876 mean difference above the observed (original) data.  Because this is greater than 0.05 you would reject that conferences has anything to do with the length of the stints a team misses the playoffs. 
...
drops <- c("next_season_win_pct","team",'made_playoffs')
df <- results[ , !(names(results) %in% drops)]
results
cor(df)

```
```{r}
drops <- c('efg','def_efg','wins','made_playoffs','games')
df1 <- results[ , !(names(results) %in% drops)]
df1

library(dplyr)
team_list = split(df1, df1$team)

for (i in team_list){
next_season_win_percent <- lm(next_season_win_pct ~ net_rtg+age+strength_of_schedule+three_pt_pct+two_pt_pct+true_shooting_pct+tov_rate+oreb+dreb+free_throw_rate+def_three_pt_pct+def_two_pt_pct+def_tov_rate+def_free_throw_rate, data = team_list[i])
coef(next_season_win_percent)
}
```
**Part (b):**  

Explain your model to a member of the front office. What are your takeaways? Please answer in no more than 4 sentenes.  
Wins very correlated with net_rtg
EFG very correlated with multiple attributes 
def_efg very correlated with def_two_pt_pct
